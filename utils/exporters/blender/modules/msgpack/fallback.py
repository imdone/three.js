...
class Unpacker(object):
    ...
    def _fb_unpack(self, execute=EX_CONSTRUCT, write_bytes=None):
        ...
        if execute == EX_READ_MAP_HEADER:
            ...
            return n
        # TODO should we eliminate the recursion? id:243
        if typ == TYPE_ARRAY:
            if execute == EX_SKIP:
                for i in xrange(n):
                    # TODO check whether we need to call `list_hook` id:227
            ...
            # TODO is the interaction between `list_hook` and `use_list` ok? id:188
            return ret if self._use_list else tuple(ret)
        if typ == TYPE_MAP:
            if execute == EX_SKIP:
                for i in xrange(n):
                    # TODO check whether we need to call hooks id:215
                    self._fb_unpack(EX_SKIP, write_bytes)
                    self._fb_unpack(EX_SKIP, write_bytes)
                return
